...
class ValidationTestUtils {
 public:
  ...
  static std::vector<double> create_edge_case_values() {
    return {1e308,   // Very large positive value (near infinity)
...
TEST(CrossComponentValidationIntegrationTests, Error_Handling_Across_Component_Boundaries) {
  ...
  try {
    ...
    // Test error recovery
    DetailedError factory_error(ErrorCode::InvalidInput, "Body factory validation failed",
                                ErrorSeverity::Error, "Cross-component validation test");
