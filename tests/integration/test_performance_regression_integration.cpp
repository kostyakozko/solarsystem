...
class PerformanceBaseline {
 ...
 public:
  static void set_baseline(const std::string& test_name,
                           const PerformanceMeasurement::Metrics& metrics) {
    baselines_[test_name] = metrics;
  }

  static bool check_regression(const std::string& test_name,
                               const PerformanceMeasurement::Metrics& current,
    ...
    bool time_regression =
        current.execution_time.count() > (baseline.execution_time.count() * threshold);
