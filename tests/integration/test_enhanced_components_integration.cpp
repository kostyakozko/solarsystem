  ...
  try {
    ...
    for (const auto& name : body_names) {
      try {
        ...
        // Test error recovery
        DetailedError error(ErrorCode::InvalidInput, "Body creation failed for " + name,
                            ErrorSeverity::Warning, "Integration test");
...
TEST(EnhancedComponentsIntegrationTests, Enhanced_System_Performance) {
  ...
  try {
    ...
    if (collection.size() > 0) {
      ...
      DetailedError test_error(ErrorCode::OperationFailed, "Test error", ErrorSeverity::Info,
                               "Performance test");
  ...
  std::cout << "Enhanced system performance test completed in " << duration.count() << "ms"
            << std::endl;
...
TEST(EnhancedComponentsIntegrationTests, Real_Scenario_Error_Recovery) {
  try {
    ...
    DetailedError test_error(ErrorCode::InvalidInput, "Test error for recovery",
                             ErrorSeverity::Warning, "Real scenario test");
