...
TEST(DataValidationIntegrationTestsTest, Recovery_from_Backup) {
  ...
  if (!primary_valid && fs::exists(backup_file)) {
    ...
    recovered_data =
        std::string((std::istreambuf_iterator<char>(backup_in)), std::istreambuf_iterator<char>());
    backup_in.close();
  }
  ...
  for (size_t i = 0; i < positions.size(); ++i) {
    if (!valid_flags[i]) {
      ...
      if (valid_flags[prev_idx] && valid_flags[next_idx]) {
        double t =
            (timestamps[i] - timestamps[prev_idx]) / (timestamps[next_idx] - timestamps[prev_idx]);
        positions[i] = positions[prev_idx] + t * (positions[next_idx] - positions[prev_idx]);
        valid_flags[i] = true;
      }
    }
  }
