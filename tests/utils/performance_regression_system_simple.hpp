...
struct PerformanceBaseline {
  ...
  [[nodiscard]] bool is_regression(const SimplePerformanceMetrics& metrics,
    ...
    double memory_ratio = baseline_memory_kb > 0
                              ? static_cast<double>(metrics.memory_usage_kb) / baseline_memory_kb
                              : 1.0;
...
class SimpleRegressionDetector {
 public:
  ...
  std::vector<RegressionAlert> detect_regressions(const std::string& component_name,
                                                  const std::string& test_name,
...
class SimplePerformanceRegressionSystem {
 ...
 private:
  ...
  std::map<std::string, std::map<std::string, std::function<SimplePerformanceMetrics()>>>
...
void initialize_simple_performance_regression_testing(
    const std::string& baseline_file = "performance_baselines.txt");
...
SimplePerformanceMetrics create_simple_metrics(const std::string& test_name,
                                               ...
                                               size_t memory_kb = 0, double cpu_percent = 0.0);
