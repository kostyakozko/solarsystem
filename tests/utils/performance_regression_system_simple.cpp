...
void SimpleBaselineManager::update_baseline(const std::string& component_name,
    ...
    baseline.baseline_memory_kb = static_cast<size_t>(
        baseline.baseline_memory_kb * (1.0 - alpha) + metrics.memory_usage_kb * alpha);
...
void SimpleBaselineManager::load_baselines(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (iss >> component_name >> test_name >> time_ms >> memory_kb >>
...
std::vector<RegressionAlert> SimpleRegressionDetector::detect_regressions(
    ...
    if (baseline->is_regression(metrics, threshold_)) {
        ...
        alert.description = "Performance regression detected: " +
                           std::to_string(alert.regression_factor) + "x slower than baseline";
...
std::vector<OptimizationRecommendation> SimpleOptimizationAnalyzer::analyze_performance(
    ...
    if (metrics.memory_usage_kb > 10 * 1024) { // > 10MB
        ...
        rec.issue_description = "High memory usage detected (" +
                               std::to_string(metrics.memory_usage_kb / 1024) + " MB)";
    ...
    if (metrics.cpu_usage_percent > 80.0) {
        ...
        rec.issue_description = "High CPU usage detected (" +
                               std::to_string(metrics.cpu_usage_percent) + "%)";
...
std::vector<OptimizationRecommendation> SimpleOptimizationAnalyzer::analyze_regression_alerts(
    ...
    for (const auto& alert : alerts) {
        ...
        rec.priority_score = alert.regression_factor > 2.0 ? 9 :
                            (alert.regression_factor > 1.5 ? 7 : 5);
        recommendations.push_back(rec);
    }
...
bool SimplePerformanceRegressionSystem::has_regressions() const {
    return !recent_alerts_.empty();
}

std::vector<OptimizationRecommendation> SimplePerformanceRegressionSystem::get_optimization_recommendations() {
