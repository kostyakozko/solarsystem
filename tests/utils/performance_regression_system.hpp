...
struct ComponentBaseline {
  ...
  [[nodiscard]] double get_regression_factor(const PerformanceMetrics& metrics) const {
    ...
    return static_cast<double>(metrics.execution_time.count()) / baseline_execution_time.count();
  }
...
struct RegressionAlert {
  ...
  enum class MetricType { EXECUTION_TIME, MEMORY_USAGE, CPU_USAGE, CACHE_PERFORMANCE, THROUGHPUT };
  ...
  [[nodiscard]] std::string metric_type_string() const {
    switch (metric_type) {
      ...
      case MetricType::CPU_USAGE:
        return "CPU Usage";
      case MetricType::CACHE_PERFORMANCE:
  ...
  [[nodiscard]] std::string category_string() const {
    switch (category) {
      case Category::ALGORITHM:
        return "Algorithm";
      case Category::MEMORY_MANAGEMENT:
        return "Memory Management";
      case Category::CPU_OPTIMIZATION:
        return "CPU Optimization";
      case Category::CACHE_OPTIMIZATION:
        return "Cache Optimization";
      case Category::IO_OPTIMIZATION:
        return "I/O Optimization";
...
class ComponentBaselineManager {
 public:
  ...
  void set_baseline(const std::string& component_name, const std::string& test_name,
                    const PerformanceMetrics& metrics);

  void update_baseline(const std::string& component_name, const std::string& test_name,
                       const PerformanceMetrics& metrics);

  [[nodiscard]] std::optional<ComponentBaseline> get_baseline(const std::string& component_name,
...
class AutomatedRegressionDetector {
 ...
 private:
  ...
  [[nodiscard]] std::string generate_recommendation(const RegressionAlert& alert) const;

  [[nodiscard]] bool is_statistically_significant(const PerformanceMetrics& metrics,
...
class PerformanceOptimizationAnalyzer {
 ...
 private:
  std::map<std::string,
           std::function<std::vector<OptimizationRecommendation>(const PerformanceMetrics&)>>
      custom_analyzers_;
...
class PerformanceMonitoringSystem {
 public:
  ...
  void register_component_test(const std::string& component_name, const std::string& test_name,
                               std::function<PerformanceMetrics()> test_function);

  void run_component_test(const std::string& component_name, const std::string& test_name);
 ...
 private:
  ...
  std::map<std::pair<std::string, std::string>, std::function<PerformanceMetrics()>>
      registered_tests_;
  std::vector<RegressionAlert> alert_history_;
  std::vector<OptimizationRecommendation> optimization_recommendations_;

  void monitoring_loop();
  void process_test_result(const std::string& component_name, const std::string& test_name,
                           const PerformanceMetrics& metrics);
...
class PerformanceRegressionTestingSystem {
 ...
 private:
  ...
  std::map<std::string, std::map<std::string, std::function<PerformanceMetrics()>>>
...
void initialize_performance_regression_testing(
    const std::string& baseline_file = "performance_baselines.json");
...
#define REGISTER_PERFORMANCE_TEST(component, test_name, test_code)                            \
  register_component_performance_tests(                                                       \
      component,                                                                              \
      {{test_name, []() {                                                                     \
...
#define PERFORMANCE_REGRESSION_TEST(component, test_name, test_code)                              \
  do {                                                                                            \
    if (g_performance_system) {                                                                   \
      g_performance_system->register_component(                                                   \
          component,                                                                              \
          {{test_name, []() {                                                                     \
              ComprehensivePerformanceMonitor::instance().start_test_monitoring(test_name);       \
              test_code;                                                                          \
