...
void ComponentBaselineManager::update_baseline(const std::string& component_name,
    ...
    baseline.baseline_execution_time = std::chrono::nanoseconds(static_cast<long long>(updated_time_ns));
...
void ComponentBaselineManager::save_baselines(const std::string& filename) const {
    ...
    file << "# Format: component_name test_name time_ns memory_kb cpu_percent cache_miss_rate ops_per_sec sample_count\n";

    for (const auto& [key, baseline] : baselines_) {
        file << baseline.component_name << " "
             << baseline.test_name << " "
             << baseline.baseline_execution_time.count() << " "
...
void ComponentBaselineManager::load_baselines(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (iss >> component_name >> test_name >> time_ns >> memory_kb >>
                   cpu_percent >> cache_miss_rate >> ops_per_sec >> sample_count) {
...
void ComponentBaselineManager::calculate_confidence_interval(ComponentBaseline& baseline) const {
    ...
    baseline.confidence_lower = std::chrono::nanoseconds(
        std::max(0LL, baseline.baseline_execution_time.count() - margin));
    baseline.confidence_upper = std::chrono::nanoseconds(
        baseline.baseline_execution_time.count() + margin);
}
...
std::vector<RegressionAlert> AutomatedRegressionDetector::detect_regressions(
    ...
    if (time_factor > config_.time_regression_threshold) {
        ...
        alert.description = "Execution time regression: " +
                           std::to_string(time_factor) + "x slower than baseline";
    ...
    if (baseline.baseline_memory_kb > 0) {
        double memory_factor = static_cast<double>(metrics.peak_memory_kb) / baseline.baseline_memory_kb;
        if (memory_factor > config_.memory_regression_threshold) {
            ...
            alert.description = "Memory usage regression: " +
                               std::to_string(memory_factor) + "x more memory than baseline";
            alert.recommendation = generate_recommendation(alert);
            alert.current_metrics = metrics;
            alert.baseline = baseline;
            alert.detected_at = std::chrono::system_clock::now();
    ...
    if (baseline.baseline_cpu_percent > 0) {
        ...
        if (cpu_factor > config_.cpu_regression_threshold) {
            ...
            alert.description = "CPU usage regression: " +
                               std::to_string(cpu_factor) + "x more CPU than baseline";
            alert.recommendation = generate_recommendation(alert);
            alert.current_metrics = metrics;
            alert.baseline = baseline;
            alert.detected_at = std::chrono::system_clock::now();
...
AutomatedRegressionDetector::DetectionConfig AutomatedRegressionDetector::get_configuration() const {
    return config_;
}

RegressionAlert::Severity AutomatedRegressionDetector::calculate_severity(
    double regression_factor, double threshold) const {
...
std::vector<OptimizationRecommendation> PerformanceOptimizationAnalyzer::analyze_memory_performance(
    ...
    if (metrics.peak_memory_kb > 100 * 1024) { // > 100MB
        ...
        rec.category = OptimizationRecommendation::Category::MEMORY_MANAGEMENT;
        rec.issue_description = "High memory usage detected (" +
                               std::to_string(metrics.peak_memory_kb / 1024) + " MB)";
        rec.recommendation = "Consider using memory pools, reducing object sizes, or implementing lazy loading";
...
std::vector<OptimizationRecommendation> PerformanceOptimizationAnalyzer::analyze_cpu_performance(
    ...
    if (metrics.cpu_stats.total_percent > 80.0) {
        ...
        rec.component_name = component_name;
        rec.category = OptimizationRecommendation::Category::CPU_OPTIMIZATION;
        rec.issue_description = "High CPU usage detected (" +
                               std::to_string(metrics.cpu_stats.total_percent) + "%)";
...
std::vector<OptimizationRecommendation> PerformanceOptimizationAnalyzer::analyze_algorithm_performance(
    const std::string& component_name,
    const PerformanceMetrics& metrics) const {
    ...
    if (metrics.execution_time_ms() > 1000.0) { // > 1 second
        ...
        rec.component_name = component_name;
        rec.category = OptimizationRecommendation::Category::ALGORITHM;
        rec.issue_description = "Long execution time detected (" +
        ...
        try {
            ...
            if (!baseline_opt.has_value()) {
                ...
                if (!alerts.empty()) {
                    ...
                    std::cout << "  ⚠️  " << alerts.size() << " regression(s) detected in " << test_name << std::endl;
                } else {
                    std::cout << "  ✅ " << test_name << " passed (" << metrics.execution_time_ms() << "ms)" << std::endl;
                }
            }
...
void PerformanceRegressionTestingSystem::load_baselines(const std::string& filename) {
    ...
    std::cout << "Loaded " << baseline_manager_->get_baseline_count() << " baselines from: " << file << std::endl;
}
...
bool PerformanceRegressionTestingSystem::has_regressions() const {
    return !recent_alerts_.empty();
}

std::vector<OptimizationRecommendation> PerformanceRegressionTestingSystem::get_optimization_recommendations() {
    recent_recommendations_ = optimization_analyzer_->analyze_regression_alerts(recent_alerts_);
    return recent_recommendations_;
}

void PerformanceRegressionTestingSystem::generate_comprehensive_report(const std::string& filename) {
...
void PerformanceRegressionTestingSystem::setup_default_analyzers() {
    ...
    optimization_analyzer_->add_custom_analyzer("JPL_Data_Loading",
    ...
    optimization_analyzer_->add_custom_analyzer("Simulation_Performance",
        [](const PerformanceMetrics& metrics) -> std::vector<OptimizationRecommendation> {
            std::vector<OptimizationRecommendation> recs;
            if (metrics.execution_time_ms() > 100) { // > 100ms for simulation step
                ...
                rec.recommendation = "Consider optimizing N-body calculations or using approximation methods";
