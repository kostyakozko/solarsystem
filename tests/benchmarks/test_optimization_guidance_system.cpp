...
class OptimizationGuidance {
 public:
  ...
  static AnalysisResult analyze_performance(
  ...
  static std::vector<BottleneckAnalysis> identify_bottlenecks_detailed(
    ...
    for (const auto& [name, time] : operation_times) {
      ...
      analysis.percentage_of_total = (total_time > 0.0) ?
                                      (time / total_time * 100.0) : 0.0;
  ...
  static ImpactAnalysis analyze_impact(
    ...
    impact.improvement_percent = (baseline_time_ms > 0.0) ?
                                 (change / baseline_time_ms * 100.0) : 0.0;
  ...
  static std::vector<PerformanceAlert> check_thresholds(
    ...
    for (const auto& [metric, value] : metrics) {
      ...
      if (value > threshold_it->second) {
        ...
        double excess_percent = ((value - threshold_it->second) /
                                threshold_it->second) * 100.0;
...
int main() {
  ...
  TEST_CASE("Performance Analysis and Recommendation Engine") {
    // Test 1.1: Optimal performance
    {
      auto result = OptimizationGuidance::analyze_performance(
          "fast_component", 50.0, 1024 * 1024, 30.0);
    ...
    {
      auto result = OptimizationGuidance::analyze_performance(
          "slow_component", 2000.0, 1024 * 1024, 30.0);
    ...
    {
      auto result = OptimizationGuidance::analyze_performance(
          "memory_heavy", 100.0, 200 * 1024 * 1024, 30.0);
    ...
    {
      auto result = OptimizationGuidance::analyze_performance(
          "problematic", 2000.0, 200 * 1024 * 1024, 85.0);
  ...
  TEST_CASE("Performance Impact Analysis for Code Changes") {
    // Test 3.1: Significant improvement
    {
      auto impact = OptimizationGuidance::analyze_impact(
          "Algorithm optimization", 1000.0, 700.0);
  ...
  TEST_CASE("Integrated Optimization Workflow") {
    // Test 5.1: Complete analysis workflow
    {
      ...
      auto analysis = OptimizationGuidance::analyze_performance(
          "body_creation", execution_time, 1024 * 1024, 50.0);
      ...
      std::map<std::string, double> operation_times = {
          {"body_creation", execution_time},
          {"other_op", 10.0}};

      auto bottlenecks = OptimizationGuidance::identify_bottlenecks_detailed(
          operation_times);
      ...
      std::map<std::string, double> metrics = {
          {"body_creation_ms", execution_time}};

      std::map<std::string, double> thresholds = {
          {"body_creation_ms", 1000.0}};
    ...
    // Test 5.2: Impact analysis workflow
    {
      ...
      // Analyze impact
      auto impact = OptimizationGuidance::analyze_impact(
          "Test change", baseline_time, new_time);
