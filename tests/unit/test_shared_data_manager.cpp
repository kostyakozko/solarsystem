...
TEST(SharedDataManagerTests, Multiple_Locks) {
  ...
  if (manager.lock("key1", "owner3"))
    throw std::runtime_error("Should not be able to lock already locked key");
...
TEST(SharedDataManagerTests, Synchronize_with_New_Remote_Data) {
  ...
  if (result.conflicts_detected != 0)
    throw std::runtime_error("Should have no conflicts with new data");
...
TEST(SharedDataManagerTests, Statistics___Initial_State) {
  ...
  if (stats.conflicts_resolved != 0)
    throw std::runtime_error("Should start with 0 conflicts resolved");
}
...
TEST(SharedDataManagerTests, Clear_All_Data) {
  ...
  if (stats.locked_entries != 0)
    throw std::runtime_error("Should have 0 locked entries after clear");
