...
class AuthSecurityTester {
 ...
 public:
  ...
  AuthResult authenticate(const std::string& username, const std::string& password) {
    ...
    session.expires_at = session.created_at + std::chrono::hours(24);  // 24 hour expiry
  ...
  AuthzResult authorize(const std::string& token, Permission permission) {
    ...
    switch (session.role) {
      ...
      case Role::User:
        ...
        result.authorized = (permission == Permission::Read || permission == Permission::Write ||
  ...
  bool test_privilege_escalation(const std::string& token, Role target_role) {
    auto it = active_sessions_.find(token);
    if (it == active_sessions_.end()) {
      return false;
    }
...
TEST(AuthenticationAndAuthorizationSecurityTestsTest, Authorization_and_Access_Control) {
  ...
  auto admin_auth = tester.authenticate("admin", "admin_pass_123");
  auto read_perm = tester.authorize(admin_auth.session_token, AuthSecurityTester::Permission::Read);
  auto write_perm =
      tester.authorize(admin_auth.session_token, AuthSecurityTester::Permission::Write);
  auto delete_perm =
      tester.authorize(admin_auth.session_token, AuthSecurityTester::Permission::Delete);
  ASSERT_TRUE(read_perm.authorized && write_perm.authorized && delete_perm.authorized);

  // Test 3.2: User permissions
  auto user_auth = tester.authenticate("user1", "user_pass_456");
  auto user_read = tester.authorize(user_auth.session_token, AuthSecurityTester::Permission::Read);
  auto user_delete =
      tester.authorize(user_auth.session_token, AuthSecurityTester::Permission::Delete);
  ...
  auto guest_read =
      tester.authorize(guest_auth.session_token, AuthSecurityTester::Permission::Read);
  auto guest_write =
      tester.authorize(guest_auth.session_token, AuthSecurityTester::Permission::Write);
  ASSERT_TRUE(guest_read.authorized);
  ASSERT_FALSE(guest_write.authorized);

  // Test 3.4: Invalid session authorization
  auto invalid_authz = tester.authorize("invalid_token", AuthSecurityTester::Permission::Read);
...
TEST(AuthenticationAndAuthorizationSecurityTestsTest, Privilege_Escalation_Testing) {
  ...
  auto user_auth = tester.authenticate("user1", "user_pass_456");
  bool escalation_blocked =
      !tester.test_privilege_escalation(user_auth.session_token, AuthSecurityTester::Role::Admin);
  ASSERT_TRUE(escalation_blocked);

  // Test 4.2: Attempt to escalate from Guest to User
  auto guest_auth = tester.authenticate("guest", "guest_pass_789");
  bool guest_escalation =
      !tester.test_privilege_escalation(guest_auth.session_token, AuthSecurityTester::Role::User);
  ASSERT_TRUE(guest_escalation);

  // Test 4.3: Same role is allowed
  bool same_role =
      tester.test_privilege_escalation(user_auth.session_token, AuthSecurityTester::Role::User);
  ASSERT_TRUE(same_role);

  // Test 4.4: Invalid token escalation
  bool invalid_escalation =
      tester.test_privilege_escalation("invalid_token", AuthSecurityTester::Role::Admin);
...
TEST(AuthenticationAndAuthorizationSecurityTestsTest, Role_Based_Access_Control) {
  ...
  // Admin can delete
  auto admin_delete = tester.authorize(admin.session_token, AuthSecurityTester::Permission::Delete);
  ASSERT_TRUE(admin_delete.authorized);

  // User cannot delete
  auto user_delete = tester.authorize(user.session_token, AuthSecurityTester::Permission::Delete);
  ASSERT_FALSE(user_delete.authorized);

  // Guest cannot write
  auto guest_write = tester.authorize(guest.session_token, AuthSecurityTester::Permission::Write);
  ASSERT_FALSE(guest_write.authorized);

  // Test 8.2: All roles can read
  auto admin_read = tester.authorize(admin.session_token, AuthSecurityTester::Permission::Read);
  auto user_read = tester.authorize(user.session_token, AuthSecurityTester::Permission::Read);
  auto guest_read = tester.authorize(guest.session_token, AuthSecurityTester::Permission::Read);
  ASSERT_TRUE(admin_read.authorized && user_read.authorized && guest_read.authorized);
}
