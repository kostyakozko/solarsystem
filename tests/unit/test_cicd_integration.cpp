...
class CIPipelineManager {
 public:
  ...
  void update_stage_status(const std::string& run_id, PipelineStage stage, PipelineStatus status) {
...
TEST(CICDPipelineIntegrationTest, Deployment_Management) {
  ...
  std::string deployment_id = manager.deploy("4.1.0", DeploymentManager::Environment::Staging);
  ...
  std::string prod_deployment = manager.deploy("4.1.0", DeploymentManager::Environment::Production);
...
TEST(CICDPipelineIntegrationTest, End_to_End_CICD_Workflow) {
  ...
  std::string staging_deploy = deployment.deploy("4.2.0", DeploymentManager::Environment::Staging);
  deployment.complete_deployment(staging_deploy, DeploymentManager::DeploymentStatus::Success);
  ASSERT_TRUE(deployment.get_deployment_status(staging_deploy) ==
              DeploymentManager::DeploymentStatus::Success);

  // Test 5.6: Deploy to production
  std::string prod_deploy = deployment.deploy("4.2.0", DeploymentManager::Environment::Production);
