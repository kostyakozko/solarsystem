...
int main() {
  ...
  TEST_CASE("Concurrent Port Allocation") {
  ...
  TEST_CASE("Concurrent Resource Cleanup") {
  ...
  TEST_CASE("Concurrent Diagnostic Logging") {
  ...
  TEST_CASE("Concurrent Performance Monitoring") {
  ...
  TEST_CASE("Mixed Operations Stress Test") {
    ...
    ASSERT_GE(success_count.load(), num_operations * 0.9);
  });
