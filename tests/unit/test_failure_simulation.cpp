...
class FailureSimulator {
 ...
 public:
  ...
  FailureResult simulate_hang(int duration_ms) {
    ...
    result.failed = true;
    result.description = "Process hang: Unresponsive for " + std::to_string(duration_ms) + "ms";

    // Actually hang for a short time (scaled down for testing)
    std::this_thread::sleep_for(std::chrono::milliseconds(std::min(duration_ms / 10, 100)));
  ...
  FailureResult simulate_service_failure(const std::string& service_name, ServiceState state) {
    FailureResult result;
  ...
  FailureResult simulate_partial_failure(double failure_rate) {
    ...
    if (random_value < failure_rate) {
      ...
      result.description = "Partial failure: Operation failed (" +
                           std::to_string(static_cast<int>(failure_rate * 100)) + "% failure rate)";
    } else {
      result.failed = false;
  ...
  std::vector<FailureResult> simulate_cascading_failures(int num_services) {
    ...
    for (int i = 0; i < num_services; ++i) {
      ...
      if (prob_dist_(rng_) < failure_prob) {
        auto result = simulate_service_failure(service_name, ServiceState::Unavailable);
  ...
  FailureResult test_recovery(FailureResult failure, std::function<bool()> recovery_func) {
    if (!failure.failed) {
...
TEST(FailureSimulationTestingTest, Network_Failure_Simulation) {
  ...
  // Test 2.1: Packet loss
  {
    ...
    ASSERT_TRUE(result.failed);
    ASSERT_TRUE(result.type == FailureSimulator::FailureType::NetworkFailure);
...
TEST(FailureSimulationTestingTest, Data_Corruption_Simulation) {
  ...
  // Test 6.1: Bit flip
  {
    ...
    ASSERT_TRUE(result.failed);
    ASSERT_TRUE(result.type == FailureSimulator::FailureType::CorruptionFailure);
...
TEST(FailureSimulationTestingTest, External_Service_Failure_Simulation) {
  ...
  {
    auto result =
        simulator.simulate_service_failure("JPL_API", FailureSimulator::ServiceState::Unavailable);
    ASSERT_TRUE(result.failed);
    ASSERT_TRUE(result.type == FailureSimulator::FailureType::ServiceUnavailable);
  ...
  {
    auto result =
        simulator.simulate_service_failure("Database", FailureSimulator::ServiceState::Degraded);
  ...
  {
    auto result =
        simulator.simulate_service_failure("Cache", FailureSimulator::ServiceState::Available);
    ASSERT_FALSE(result.failed);
