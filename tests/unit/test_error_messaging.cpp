...
TEST(ErrorMessagingTests, Format_Error_Simple) {
  ...
  if (formatted.find("Format Test") == std::string::npos)
    throw std::runtime_error("Title not in output");
}
TEST(ErrorMessagingTests, Format_Error_Detailed) {
  ...
  if (formatted.find("Detailed Error") == std::string::npos)
    throw std::runtime_error("Title not in output");
  if (formatted.find("Try this fix") == std::string::npos)
    throw std::runtime_error("Recovery action not in output");
}
TEST(ErrorMessagingTests, Format_Error_JSON) {
  ...
  if (json.find("\"error_code\"") == std::string::npos)
    throw std::runtime_error("JSON format invalid");
}
TEST(ErrorMessagingTests, Error_Reporting) {
  ...
  if (history[0].error_code != "REPORT001")
    throw std::runtime_error("Error code mismatch in history");
}
TEST(ErrorMessagingTests, Error_Statistics) {
  ...
  if (messaging.get_error_count_by_severity(ErrorSeverity::ERROR) != 2)
    throw std::runtime_error("Error severity count mismatch");
  if (messaging.get_error_count_by_category(ErrorCategory::CONFIGURATION) != 2)
    throw std::runtime_error("Error category count mismatch");
}
TEST(ErrorMessagingTests, Recovery_Actions) {
  ...
  if (actions[0].description != "Action 1")
    throw std::runtime_error("Recovery action description mismatch");
}
...
TEST(ErrorMessagingTests, Error_Report_Generation) {
  ...
  if (report.find("Report Generation Test") == std::string::npos)
    throw std::runtime_error("Title not in report");
}
