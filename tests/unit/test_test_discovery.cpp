...
TEST(TestDiscovery, TestRegistrationAndDiscovery) {
...
TEST(TestDiscovery, PatternMatching) {
  // Clear any existing registrations
  TestDiscovery::instance().clear_registry();
...
TEST(TestDiscovery, AdvancedFilteringWithDiscoveryOptions) {
  // Clear any existing registrations
  TestDiscovery::instance().clear_registry();
...
TEST(TestDiscovery, TestInformationRetrieval) {
  // Clear any existing registrations
  TestDiscovery::instance().clear_registry();
  ...
  ASSERT_NE(nullptr, test_info.get());
...
TEST(TestDiscovery, TestScannerFunctionality) {
