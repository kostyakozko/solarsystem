...
TEST(ConfigurationManagerTests, Configuration_Source_Tracking) {
  ...
  if (source != ConfigSource::Default) throw std::runtime_error("Initial source should be Default");
}
...
TEST(ConfigurationManagerTests, Configuration_Backup_and_Restore) {
  ...
  if (backup.description != "test backup") throw std::runtime_error("Backup description wrong");
