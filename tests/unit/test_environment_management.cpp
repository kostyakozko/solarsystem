...
class TestEnvironmentManager {
 ...
 public:
  ...
  bool create_file(const std::string& env_name, const std::string& filename,
    ...
    try {
      std::string filepath = it->second.environment_path + "/data/" + filename;
  ...
  std::string read_file(const std::string& env_name, const std::string& filename) {
    auto it = environments_.find(env_name);
    if (it == environments_.end()) {
      return "";
    }
  ...
  ResourceUsage get_resource_usage(const std::string& env_name) {
    ...
    try {
      ...
      for (const auto& entry : fs::recursive_directory_iterator(env_path)) {
        if (fs::is_regular_file(entry)) {
          usage.file_count++;
          usage.disk_mb += static_cast<size_t>(fs::file_size(entry)) / (1024 * 1024);
        }
      }
  ...
  bool check_limits(const std::string& env_name, const EnvironmentConfig& config) {
...
TEST(TestEnvironmentManagementTestsTest, Data_Isolation_and_Sandboxing) {
  ...
  {
    ASSERT_TRUE(manager.create_file("isolated_1", "data1.txt", "Content 1"));
    ASSERT_TRUE(manager.create_file("isolated_2", "data2.txt", "Content 2"));
  }
...
TEST(TestEnvironmentManagementTestsTest, State_Management_and_Reset) {
  ...
  // Test 3.1: Create and populate environment
  {
    ...
    ASSERT_TRUE(manager.create_file("stateful_env", "file1.txt", "Data 1"));
    ASSERT_TRUE(manager.create_file("stateful_env", "file2.txt", "Data 2"));
    ASSERT_TRUE(manager.create_file("stateful_env", "file3.txt", "Data 3"));
  ...
  {
    ASSERT_TRUE(manager.create_file("stateful_env", "new_file.txt", "New data"));
    std::string content = manager.read_file("stateful_env", "new_file.txt");
...
TEST(TestEnvironmentManagementTestsTest, Environment_Lifecycle) {
  ...
  // Test 6.1: Full lifecycle
  {
    ...
    std::string content = manager.read_file("lifecycle_env", "new_work.txt");
    ASSERT_EQ(content, "New data");
