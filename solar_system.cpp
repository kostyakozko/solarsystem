...
int main()
{
  ...
  while(1)
  {
    for (int i = 0; i < 10; i++)
    {
      ...
      SolarSystem[i].position.x += (SolarSystem[i].speed.x - (delta.x / 2)) * dt;
      SolarSystem[i].position.y += (SolarSystem[i].speed.y - (delta.y / 2)) * dt;
      SolarSystem[i].position.z += (SolarSystem[i].speed.z - (delta.z / 2)) * dt;
