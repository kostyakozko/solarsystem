...
RobustFileStream::RobustFileStream(const std::string& filename, Mode mode,
  ...
  if (!opened) {
    result_.error_message = "Failed to open file after " + std::to_string(attempt + 1) + " attempts";
  }
...
bool RobustFileStream::attempt_open(const std::string& path) {
  try {
    ...
    if (mode_ == Mode::Read) {
      ...
      std::string target_path = config_.use_atomic_writes && !temp_file_path_.empty()
...
void RobustFileStream::setup_atomic_write(const std::string& final_path) {
  ...
  std::string temp_name = final_file_path.filename().string() + ".tmp." +
...
FileOperationResult RobustFileHandler::write_file(const std::string& filename, const std::string& content) {
...
FileOperationResult RobustFileHandler::append_file(const std::string& filename, const std::string& content) {
...
FileOperationResult RobustFileHandler::read_file(const std::string& filename, std::string& content) {
...
std::unique_ptr<RobustFileStream> RobustFileHandler::create_output_stream(const std::string& filename) {
  return std::make_unique<RobustFileStream>(filename, RobustFileStream::Mode::Write, config_);
}

std::unique_ptr<RobustFileStream> RobustFileHandler::create_input_stream(const std::string& filename) {
  return std::make_unique<RobustFileStream>(filename, RobustFileStream::Mode::Read, config_);
}

std::unique_ptr<RobustFileStream> RobustFileHandler::create_append_stream(const std::string& filename) {
  return std::make_unique<RobustFileStream>(filename, RobustFileStream::Mode::Append, config_);
}

template<typename Operation>
FileOperationResult RobustFileHandler::retry_operation(const std::string& description, Operation&& op) {
  auto start_time = std::chrono::steady_clock::now();
...
void RobustFileHandler::log_operation_attempt(const std::string& operation, const std::string& path,
  ...
  if (attempt > 0 || !error.empty()) {
    std::cerr << "[RobustFileHandler] " << operation << " attempt " << (attempt + 1)
              << " on " << path;
    if (!error.empty()) {
...
bool TemporaryFile::exists() const {
