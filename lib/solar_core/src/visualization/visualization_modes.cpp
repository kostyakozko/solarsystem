...
static Utils::Expected<std::string, VisualizationFrame::ExportError> render_with_cairo(
    const VisualizationFrame& frame, ExportFormat format,
...
Utils::Expected<std::string, VisualizationFrame::ExportError> VisualizationFrame::export_to(
    ExportFormat format, const std::map<std::string, std::string>& options) const {
...
#else
      return Utils::Expected<std::string, ExportError>(ExportError(
          "PNG/PDF export requires Cairo library. "
...
Utils::Expected<VisualizationFrame, std::string> VisualizationRenderer::render_table(
  ...
  if (config_.data_options.show_distances) {
    output << std::string(static_cast<size_t>(config_.max_width - 2), '-') << "\n";
    output << "Total bodies: " << filtered_data.size() << " | Quality: " << std::fixed
           << std::setprecision(1) << (snapshot.overall_quality * 100.0) << "%\n";
  }
...
Utils::Expected<VisualizationFrame, std::string> VisualizationRenderer::render_grid(
  ...
  int rows = static_cast<int>((filtered_data.size() + static_cast<size_t>(cols) - 1) /
                              static_cast<size_t>(cols));

  if (config_.layout == DisplayLayout::GRID_2X2) {
    cols = 2;
    rows = static_cast<int>((filtered_data.size() + static_cast<size_t>(cols) - 1) /
                            static_cast<size_t>(cols));
  }
...
Utils::Expected<VisualizationFrame, std::string> VisualizationRenderer::render_detailed(
  ...
  for (const auto& point : filtered_data) {
    ...
    if (config_.data_options.show_distances) {
      double distance = static_cast<double>(std::sqrt(point.position.x() * point.position.x() +
                                                      point.position.y() * point.position.y() +
                                                      point.position.z() * point.position.z()));
      output << "Distance from origin: " << format_distance(distance) << "\n";
    }

    output << "Quality: " << std::fixed << std::setprecision(1) << (point.quality_score * 100.0)
           << "%\n";
...
Utils::Expected<VisualizationFrame, std::string> VisualizationRenderer::render_dashboard(
  ...
  output << "| Bodies: " << std::setw(panel_width - 9) << std::left << filtered_data.size() << "|";
  ...
  for (int i = 0; i < max_bodies; ++i) {
    std::string body_line = filtered_data[static_cast<size_t>(i)].body_name.substr(
        0, static_cast<size_t>(panel_width - 2));
    ...
    if (i == 0) {
      ...
      output << " Missing: " << std::setw(panel_width - 10) << std::left << snapshot.missing_bodies
             << "|\n";
    } else {
      output << std::string(static_cast<size_t>(panel_width), ' ') << "|\n";
    }
...
Utils::Expected<VisualizationFrame, std::string> VisualizationRenderer::render_tree(
  ...
  for (const auto& point : filtered_data) {
    ...
    if (point.body_name == "Sun") {
      category = "Star";
    } else if (point.body_name.find("Moon") != std::string::npos || point.body_name == "Phobos" ||
               point.body_name == "Deimos" || point.body_name == "Io" ||
               point.body_name == "Europa" || point.body_name == "Ganymede" ||
               point.body_name == "Callisto") {
      category = "Moons";
    } else {
      category = "Planets";
    }
...
Utils::Expected<VisualizationFrame, std::string> VisualizationRenderer::render_chart(
  ...
  for (const auto& [name, distance] : distances) {
    int bar_width =
        max_distance > 0 ? static_cast<int>((distance / max_distance) * max_bar_width) : 0;
...
std::string VisualizationRenderer::format_position(const Math::Vector3d& pos) const {
  ...
  if (config_.data_options.position_units == "km") {
    ...
    ss << "(" << (pos.x() / 1000.0 / AU) << ", " << (pos.y() / 1000.0 / AU) << ", "
       << (pos.z() / 1000.0 / AU) << ")";
  } else {
...
std::vector<Streaming::DataPoint> VisualizationRenderer::filter_and_sort_data(
  ...
  if (current_sort_field_ == "name") {
    std::sort(filtered_data.begin(), filtered_data.end(), [this](const auto& a, const auto& b) {
...
std::string VisualizationRenderer::create_table_header() const {
  ...
  if (config_.data_options.show_positions) {
    header << std::setw(45) << std::left
           << "Position (" + config_.data_options.position_units + ")";
  }

  if (config_.data_options.show_velocities) {
    header << std::setw(35) << std::left
           << "Velocity (" + config_.data_options.velocity_units + ")";
  }
...
Utils::Expected<VisualizationFrame, std::string> VisualizationRenderer::render_bodies(
    const Bodies::BodyCollection& bodies, std::chrono::system_clock::time_point timestamp) {
...
Utils::Expected<void, std::string> VisualizationRenderer::handle_input(char key, int x, int y) {
  ...
  if (it == config_.controls.keyboard_shortcuts.end()) {
    return Utils::Expected<void, std::string>(std::string("Unknown keyboard shortcut: ") + key);
  }
  ...
  return Utils::Expected<void, std::string>(std::string("Action not implemented: ") + action);
}
...
VisualizationRenderer::export_current_view(ExportFormat format,
                                           const std::map<std::string, std::string>& options) {
...
Utils::Expected<std::string, VisualizationFrame::ExportError>
VisualizationRenderer::create_share_url(const VisualizationFrame& frame,
  ...
  if (options.include_timestamp) {
    url << "timestamp="
        << std::chrono::duration_cast<std::chrono::seconds>(frame.timestamp.time_since_epoch())
               .count()
        << "&";
  }
...
std::string VisualizationRenderer::apply_color_scheme(const std::string& text,
...
std::string to_string(DisplayLayout layout) {
  switch (layout) {
    case DisplayLayout::SINGLE_COLUMN:
      return "single_column";
    case DisplayLayout::MULTI_COLUMN:
      return "multi_column";
    case DisplayLayout::HORIZONTAL:
      return "horizontal";
    case DisplayLayout::VERTICAL:
      return "vertical";
    case DisplayLayout::GRID_2X2:
      return "grid_2x2";
    case DisplayLayout::GRID_3X3:
      return "grid_3x3";
    case DisplayLayout::ADAPTIVE:
      return "adaptive";
    case DisplayLayout::CUSTOM:
      return "custom";
    default:
      return "unknown";
  }
...
Utils::Expected<VisualizationMode, std::string> parse_visualization_mode(
    const std::string& mode_str) {
  if (mode_str == "table")
    return Utils::Expected<VisualizationMode, std::string>(VisualizationMode::TABLE);
  if (mode_str == "grid")
    return Utils::Expected<VisualizationMode, std::string>(VisualizationMode::GRID);
  if (mode_str == "list")
    return Utils::Expected<VisualizationMode, std::string>(VisualizationMode::LIST);
  if (mode_str == "tree")
    return Utils::Expected<VisualizationMode, std::string>(VisualizationMode::TREE);
  if (mode_str == "chart")
    return Utils::Expected<VisualizationMode, std::string>(VisualizationMode::CHART);
  if (mode_str == "dashboard")
    return Utils::Expected<VisualizationMode, std::string>(VisualizationMode::DASHBOARD);
  if (mode_str == "minimal")
    return Utils::Expected<VisualizationMode, std::string>(VisualizationMode::MINIMAL);
  if (mode_str == "detailed")
    return Utils::Expected<VisualizationMode, std::string>(VisualizationMode::DETAILED);
  if (mode_str == "custom")
    return Utils::Expected<VisualizationMode, std::string>(VisualizationMode::CUSTOM);
...
Utils::Expected<ExportFormat, std::string> parse_export_format(const std::string& format_str) {
  ...
  if (format_str == "markdown")
    return Utils::Expected<ExportFormat, std::string>(ExportFormat::MARKDOWN);
