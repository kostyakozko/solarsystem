...
/**
 * @brief Modern 3D vector class with constexpr support
 *
...
class Vector3 {
 ...
 public:
  ...
  constexpr bool operator!=(const Vector3& other) const noexcept { return !(*this == other); }
...
template <typename T>
[[nodiscard]] std::string to_string(const Vector3<T>& vec);
