 ...
 */
enum class NetworkProtocol { HTTP, HTTPS, TCP, UDP };

/**
 * @brief Network security level
 */
enum class SecurityLevel { NONE, BASIC, STANDARD, HIGH, MAXIMUM };
...
class SOLAR_CORE_API EncryptionManager {
 public:
  ...
  std::optional<std::string> encrypt(const std::string& data, const std::string& key) const;
...
class SOLAR_CORE_API NetworkSecurity {
 public:
  ...
  NetworkSecurityResult validate_connection(const ConnectionInfo& conn_info) const;
...
class SOLAR_CORE_API SecureNetworkOperations {
 public:
  static SOLAR_CORE_API std::optional<std::string> secure_request(const std::string& url,
