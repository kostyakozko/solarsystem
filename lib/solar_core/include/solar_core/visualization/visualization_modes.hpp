...
class SOLAR_CORE_API VisualizationRenderer {
 public:
  ...
  [[nodiscard]] Utils::Expected<void, std::string> handle_input(char key, int x = -1, int y = -1);
  ...
  [[nodiscard]] Utils::Expected<std::string, VisualizationFrame::ExportError> export_current_view(
      ExportFormat format, const std::map<std::string, std::string>& options = {});

  [[nodiscard]] Utils::Expected<std::string, VisualizationFrame::ExportError> create_share_url(
      const VisualizationFrame& frame, const SharingOptions& options = {});
 ...
 private:
  ...
  // Helper methods
  [[nodiscard]] std::string apply_color_scheme(const std::string& text,
...
class SOLAR_CORE_API VisualizationModeManager {
 public:
  ...
  // Presets
  [[nodiscard]] Utils::Expected<void, std::string> save_preset(const std::string& name,
...
[[nodiscard]] SOLAR_CORE_API Utils::Expected<VisualizationMode, std::string>
parse_visualization_mode(const std::string& mode_str);
