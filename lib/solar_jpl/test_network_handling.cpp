...
int main() {
  ...
  config.fallback_endpoints = {"https://ssd-api.jpl.nasa.gov/horizons.api",
  ...
  try {
    ...
    std::cout << "   Network health score: " << std::fixed << std::setprecision(2) << health_score
              << " ("
    ...
    for (const auto& [url, expected] : test_urls) {
      ...
      std::cout << "   URL '" << url << "': " << (is_valid ? "Valid" : "Invalid")
                << (is_valid == expected ? " ✓" : " ✗") << "\n";
    }
